...
public class ManageModel : PageModel
{
    ...
    public async Task OnGetAsync(string? status = null, string? urgency = null, string? property = null,
        int pageNumber = 1)
    {
        try
        {
            ...
            {
                _logger.LogWarning("No superintendent email found in claims for user {UserId}",
    ...
    public string GetRowClass(TenantRequestSummaryViewModel request)
    {
        ...
        if (request.Status == "Completed" || request.Status == "Closed" ||
            request.Status == "Done") return ""; // No special highlighting for completed requests

        if (request.ScheduledDate.HasValue && request.ScheduledDate < DateTime.Now) return "table-danger"; // Overdue

        if (request.IsEmergency || request.UrgencyLevel == "Critical" || request.UrgencyLevel == "High")
            return "table-warning"; // Emergency

        if (request.Status == "Submitted") return "table-info"; // Pending assignment
