...
public class RescheduleModel : PageModel
{
    ...
    public IActionResult OnGet(Guid id)
    {
        ...
        _logger.LogInformation("User {UserEmail} with role claim {RoleClaim} attempting to reschedule request {RequestId}", 
        ...
        _logger.LogInformation("Role checks: PropertySuperintendent={PropertySuperintendent}, SystemAdmin={SystemAdmin}", 
        ...
        {
            _logger.LogWarning("User {UserEmail} with role {RoleClaim} denied access to reschedule request {RequestId} - insufficient permissions", 
                userEmail, roleClaim, id);
            
            // Set appropriate error message based on user role
            if (User.IsInRole("Worker"))
            {
                TempData["ErrorMessage"] = "Workers cannot reschedule requests. Please contact your property superintendent if the schedule needs to be changed.";
            }
            else if (User.IsInRole("Tenant"))
            {
                TempData["ErrorMessage"] = "Tenants cannot reschedule work assignments. Please contact your property management office if you need to change the scheduled work time.";
            }
            else
            {
                TempData["ErrorMessage"] = $"You don't have permission to reschedule requests. Your current role is '{roleClaim}'. Only property superintendents and system administrators can reschedule work assignments.";
            }
        ...
        TempData["RescheduleContext"] = "This request is being rescheduled. Update the worker assignment and scheduled date as needed.";
