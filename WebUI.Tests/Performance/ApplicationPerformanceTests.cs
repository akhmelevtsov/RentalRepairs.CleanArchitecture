...
public class ApplicationPerformanceTests : IClassFixture<WebApplicationTestFactory<Program>>
{
    ...
    public void DependencyInjection_Should_Resolve_Services_Quickly()
    {
        ...
        try
        {
            var logger = scope.ServiceProvider.GetService<Microsoft.Extensions.Logging.ILogger<ApplicationPerformanceTests>>();
        ...
        try
        {
            var currentUser = scope.ServiceProvider.GetService<RentalRepairs.Application.Common.Interfaces.ICurrentUserService>();
    ...
    public async Task Concurrent_Requests_Should_Perform_Well()
    {
        ...
        stopwatch.ElapsedMilliseconds.Should().BeLessThan(20000); // Should handle 10 concurrent requests in under 20 seconds
