...
public class CleanArchitectureBoundaryTests
{
    ...
    public void WebUI_Should_Not_Reference_Internal_Infrastructure_Types()
    {
        ...
        foreach (var webUIType in webUITypes)
            try
            {
                ...
                // Check field types
                var fields = webUIType.GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance |
                                                 BindingFlags.Static);
                foreach (var field in fields)
                    CheckTypeForInfrastructureViolations(field.FieldType, internalInfrastructureTypes, violations,
                        $"field {field.Name} in {webUIType.Name}");

                // Check property types
                var properties = webUIType.GetProperties(BindingFlags.NonPublic | BindingFlags.Public |
                                                         BindingFlags.Instance | BindingFlags.Static);
                foreach (var property in properties)
                    CheckTypeForInfrastructureViolations(property.PropertyType, internalInfrastructureTypes, violations,
                        $"property {property.Name} in {webUIType.Name}");

                // Check method parameters and return types
                var methods = webUIType.GetMethods(BindingFlags.NonPublic | BindingFlags.Public |
                                                   BindingFlags.Instance | BindingFlags.Static |
                                                   BindingFlags.DeclaredOnly);
                foreach (var method in methods)
                {
                    // Check return type
                    CheckTypeForInfrastructureViolations(method.ReturnType, internalInfrastructureTypes, violations,
                        $"return type of method {method.Name} in {webUIType.Name}");

                    // Check parameter types
                    var parameters = method.GetParameters();
                    foreach (var parameter in parameters)
                        CheckTypeForInfrastructureViolations(parameter.ParameterType, internalInfrastructureTypes,
                            violations, $"parameter {parameter.Name} in method {method.Name} of {webUIType.Name}");
                }
            }
            catch (Exception ex)
            {
        ...
        if (violations.Any())
        {
            ...
            if (violations.Count > 10) _output.WriteLine($"  ... and {violations.Count - 10} more violations");
        }
        else
        {
        ...
        violations.Should()
            .BeEmpty(
                "WebUI should not directly reference internal Infrastructure types. Use Application interfaces instead.");
    }
    ...
    public void WebUI_Should_Only_Use_Infrastructure_AddInfrastructure_Method()
    {
            ...
            try
            {
                ...
                foreach (var method in methods)
                {
                    ...
                    if (methodName.Contains("infrastructure") &&
                        ...
                        violations.Add(
                            $"Method {method.Name} in {webUIType.Name} may be directly using Infrastructure types");
                }
            }
            catch (Exception ex)
            {
        ...
        violations.Should()
            .BeEmpty("WebUI should only use AddInfrastructure and InitializeDemoDataAsync methods from Infrastructure");
    }
    ...
    public void Infrastructure_DependencyInjection_Should_Be_Only_Public_API()
    {
        // Arrange
        var infrastructureAssembly = Assembly.Load("RentalRepairs.Infrastructure");
        var dependencyInjectionType =
            infrastructureAssembly.GetType("RentalRepairs.Infrastructure.DependencyInjection");
        ...
        foreach (var method in publicMethods) _output.WriteLine($"  - {method.Name}");
    ...
    private void CheckTypeForInfrastructureViolations(Type typeToCheck, List<Type> internalInfrastructureTypes,
        List<string> violations, string context)
    {
        ...
        if (typeToCheck.IsGenericType)
        {
            ...
            foreach (var genericArg in genericArguments)
                CheckTypeForInfrastructureViolations(genericArg, internalInfrastructureTypes, violations,
                    $"generic argument in {context}");
        }

        // Handle array types
        if (typeToCheck.IsArray)
        {
            CheckTypeForInfrastructureViolations(typeToCheck.GetElementType()!, internalInfrastructureTypes, violations,
                $"array element in {context}");
            return;
        }
