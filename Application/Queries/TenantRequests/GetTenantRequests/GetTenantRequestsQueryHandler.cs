...
public class GetTenantRequestsQueryHandler : IRequestHandler<GetTenantRequestsQuery, List<TenantRequestDto>>
{
    ...
    public async Task<List<TenantRequestDto>> Handle(GetTenantRequestsQuery request,
        CancellationToken cancellationToken)
    {
        var query = _context.TenantRequests.AsQueryable();

        // Apply filters
        if (request.TenantId.HasValue) query = query.Where(tr => tr.TenantId == request.TenantId.Value);

        // ? Fix status filtering - convert enum to string for comparison
        if (request.Status.HasValue) query = query.Where(tr => tr.Status == request.Status.Value);

        if (request.PropertyId.HasValue) query = query.Where(tr => tr.PropertyId == request.PropertyId.Value);

        if (request.IsEmergencyOnly == true) query = query.Where(tr => tr.IsEmergency);

        // Apply sorting
        query = request.SortBy?.ToLower() switch
        {
            "title" => request.SortDescending ? query.OrderByDescending(tr => tr.Title) : query.OrderBy(tr => tr.Title),
        ...
        var tenantRequests = await query
            ...
            .Select(joined => new TenantRequestDto
            {
                ...
                SubmittedDate = joined.TenantRequest.Status.ToString() != "Draft"
                    ? joined.TenantRequest.CreatedAt
                    : (DateTime?)null,
